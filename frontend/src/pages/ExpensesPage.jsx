...
function ExpensesPage() {
  ...
  const filteredExpenses = normalizedExpenses.filter((expense) => {
    ...
    const matchesStatus =
      statusFilter === "all" || status.type === statusFilter;

    const matchesGroup =
      groupFilter === "all" || expense.group === groupFilter;
            ...
            filteredExpenses.map((expense) => {
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    setExpandedExpense(
                      expandedExpense === `${expense.group}-${expense.id}`
                        ? null
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      ...
                      {status.type === "owed" && status.debtors.length > 1 && (
                        <p className="text-green-600 text-sm">
                          {status.debtors.length} people owe you ₹
                          {status.share.toFixed(0)} each
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  {expandedExpense === `${expense.group}-${expense.id}` &&
                    status.type === "owed" &&
                    status.debtors.length > 1 && (
                      <div className="mt-4 pt-4 border-t">
                        <p className="font-medium mb-2">Details</p>

                        {status.debtors.map((person) => (
                          <p key={person} className="text-green-600 text-sm">
                            {person} owes you ₹{status.share.toFixed(0)}
                          </p>
                        ))}
                      </div>
                    )}
